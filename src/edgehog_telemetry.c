    ...
    edgehog_device_handle_t edgehog_device, edgehog_telemetry_t *edgehog_telemetry)
{
    ...
    if (strcmp(endpoint, "enable") == 0) {
        bool enable;
        if (event_request->bson_value && event_request->bson_element.type == BSON_TYPE_BOOLEAN) {
        ...
        if (event_request->bson_value && event_request->bson_element.type >= BSON_TYPE_INT32) {
